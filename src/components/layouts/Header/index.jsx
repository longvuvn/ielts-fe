...
import { Menu, LogOut, BookOpen, LayoutDashboard, Home, User, Bell, Settings } from "lucide-react";
          ...
          {navItems.map((item) => (
            (!item.private || isAuthenticated) && (
              <Link
                key={item.path}
                to={item.path}
                className={`flex items-center gap-2 px-6 py-2.5 rounded-[14px] text-sm font-bold transition-all duration-300 ${
                  isActive(item.path)
          ...
          {isAuthenticated ? (
            <div className="flex items-center gap-4 pl-4 border-l border-slate-100">
              <div className="hidden lg:block text-right">
                <p className="text-xs font-bold text-slate-900 leading-tight">{user?.name || "Người dùng"}</p>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">{user?.role || "Premium Learner"}</p>
              </div>
              
              <div className="relative" ref={userMenuRef}>
                <button 
                  onClick={() => setIsUserMenuOpen(!isUserMenuOpen)}
                  className={`relative group/avatar flex items-center justify-center w-11 h-11 rounded-2xl transition-all duration-500 ${
                    isUserMenuOpen 
                ...
                >
                  ...
                  <div className={`absolute inset-0 rounded-2xl bg-gradient-to-tr from-accent to-blue-400 opacity-20 ${isUserMenuOpen ? 'opacity-100' : 'group-hover/avatar:opacity-40'} transition-opacity`} />
                  ...
                  <div className="absolute inset-[2px] rounded-[14px] bg-white flex items-center justify-center overflow-hidden border border-slate-100 shadow-sm z-10">
                    {user?.avatar ? (
                      ...
                      <div className="w-full h-full bg-blue-50 flex items-center justify-center group-hover/avatar:bg-blue-100 transition-all">
                ...
                {/* User Dropdown */}
                <div 
                  className={`absolute right-0 top-full mt-4 w-64 bg-white border border-slate-100 rounded-[24px] shadow-[0_20px_50px_rgba(0,0,0,0.1)] p-2 z-[1001] transition-all duration-300 ease-out ${
                    isUserMenuOpen 
                ...
                >
                  ...
                  <div className="px-4 py-4 mb-1">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-blue-50 flex items-center justify-center text-accent text-sm font-black border border-blue-100">
                        {user?.name?.charAt(0) || "U"}
                      </div>
                      <div className="flex flex-col min-w-0">
                        <span className="text-sm font-bold text-slate-900 truncate">{user?.name || "Người dùng"}</span>
                  ...
                  <div className="space-y-1 border-t border-slate-50 pt-2">
                    {[
                      { icon: User, text: "Chỉnh sửa thông tin" },
                      { icon: Bell, text: "Thông báo" },
                      { icon: BookOpen, text: "Lịch sử luyện tập" },
                      { icon: Settings, text: "Cài đặt" },
                    ].map((item, idx) => (
                      <div key={idx} className="flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-bold text-slate-500 hover:text-accent hover:bg-blue-50/50 transition-all cursor-pointer group">
